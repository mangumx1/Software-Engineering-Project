...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if(strncmp(reqln[0], "GET\0", 4) == 0) {
			...
			/* Added the O_BINARY, 0644 peramiter to open for binary files and pictures*/
#if defined(WIN32)
			if((page = open(path, O_RDONLY | O_BINARY, 0644)) != -1) {
...
if((page = open(path, O_RDONLY)) != -1) {
				...
				printf("file: %s\n\n", path);
#if defined(WIN32)
				page = open(path, O_RDONLY | O_BINARY, 0644);
#elif defined(LINUX)
